...
class TimeLimit(gym.Wrapper):
    # trigger if the steps of action is limited
    # INFO: not used in boxing
...
class NoopResetEnv(gym.Wrapper):
    # applies a random amount of NOOP operations to the game reset.
...
class ClipRewardEnv(gym.RewardWrapper):
    # clips the reward to a –1…1 range
    # INFO: if we test only one game, this one should be false
    def __init__(self, env):
...
class ImageToPyTorch(gym.ObservationWrapper):
    """
    Change image shape to CWH(Tensorflow tensor is WHC)
    """
    def __init__(self, env):
...
def wrap_a2c(env, stack_frames=4, episodic_life=True, reward_clipping=True):
